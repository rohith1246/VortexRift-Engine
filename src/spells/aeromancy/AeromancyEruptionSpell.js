/**
 * AeromancyEruptionSpell.js - Tactical Combat Spell Specification
 * School: aeromancy | Form: Eruption
 */

class AeromancyEruptionSpell {
  constructor(config = {}) {
    this.spellId = 'spell_aeromancy_eruption';
    this.name = 'AeromancyEruption';
    this.school = 'aeromancy';
    this.form = 'Eruption';
    this.tier = 4;
    this.baseManaCost = config.mana || 100;
    this.staminaCost = config.stamina || 15;
    this.cooldownTicks = config.cooldown || 10;
    this.currentCooldown = 0;
    this.baseDamage = config.damage || 272;
    this.projectileSpeed = 21;
    this.aoeRadius = 4.5;
    this.statusEffect = 'status_aeromancy_eruption';
...
module.exports = { AeromancyEruptionSpell };
