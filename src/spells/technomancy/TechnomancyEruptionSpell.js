/**
 * TechnomancyEruptionSpell.js - Tactical Combat Spell Specification
 * School: technomancy | Form: Eruption
 */

class TechnomancyEruptionSpell {
  constructor(config = {}) {
    this.spellId = 'spell_technomancy_eruption';
    this.name = 'TechnomancyEruption';
    this.school = 'technomancy';
    this.form = 'Eruption';
    this.tier = 4;
    this.baseManaCost = config.mana || 115;
    this.staminaCost = config.stamina || 15;
    this.cooldownTicks = config.cooldown || 10;
    this.currentCooldown = 0;
    this.baseDamage = config.damage || 308;
    this.projectileSpeed = 21;
    this.aoeRadius = 4.5;
    this.statusEffect = 'status_technomancy_eruption';
...
module.exports = { TechnomancyEruptionSpell };
