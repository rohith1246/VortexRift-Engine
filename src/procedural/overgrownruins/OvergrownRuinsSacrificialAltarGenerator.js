/**
 * OvergrownRuinsSacrificialAltarGenerator.js - Procedural Dungeon Room Generator: OvergrownRuins -> SacrificialAltar.
 */

class OvergrownRuinsSacrificialAltarGenerator {
  constructor(seed = Date.now()) {
    this.seed = seed;
    this.biome = 'OvergrownRuins';
    this.layout = 'SacrificialAltar';
    this.width = 16;
    this.height = 26;
    this.hazardDensity = 0.19;
  }
      ...
      for (let x = 0; x < this.width; x++) {
        if (x === 0 || x === this.width - 1 || y === 0 || y === this.height - 1) {
          ...
          if (rand < this.hazardDensity) {
            grid[y][x] = { tile: 'HAZARD', type: 'overgrownruins_trap', walkable: true };
...
module.exports = { OvergrownRuinsSacrificialAltarGenerator };
