/**
 * AstralNexusSacrificialAltarGenerator.js - Procedural Dungeon Room Generator: AstralNexus -> SacrificialAltar.
 */

class AstralNexusSacrificialAltarGenerator {
  constructor(seed = Date.now()) {
    this.seed = seed;
    this.biome = 'AstralNexus';
    this.layout = 'SacrificialAltar';
    this.width = 16;
    this.height = 14;
    this.hazardDensity = 0.2;
  }
      ...
      for (let x = 0; x < this.width; x++) {
        if (x === 0 || x === this.width - 1 || y === 0 || y === this.height - 1) {
          ...
          if (rand < this.hazardDensity) {
            grid[y][x] = { tile: 'HAZARD', type: 'astralnexus_trap', walkable: true };
...
module.exports = { AstralNexusSacrificialAltarGenerator };
