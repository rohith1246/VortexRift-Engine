/**
 * ThunderPeakSacrificialAltarGenerator.js - Procedural Dungeon Room Generator: ThunderPeak -> SacrificialAltar.
 */

class ThunderPeakSacrificialAltarGenerator {
  constructor(seed = Date.now()) {
    this.seed = seed;
    this.biome = 'ThunderPeak';
    this.layout = 'SacrificialAltar';
    this.width = 16;
    this.height = 23;
    this.hazardDensity = 0.23;
  }
      ...
      for (let x = 0; x < this.width; x++) {
        if (x === 0 || x === this.width - 1 || y === 0 || y === this.height - 1) {
          ...
          if (rand < this.hazardDensity) {
            grid[y][x] = { tile: 'HAZARD', type: 'thunderpeak_trap', walkable: true };
...
module.exports = { ThunderPeakSacrificialAltarGenerator };
