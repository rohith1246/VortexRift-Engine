/**
 * ShadowLabyrinthSacrificialAltarGenerator.js - Procedural Dungeon Room Generator: ShadowLabyrinth -> SacrificialAltar.
 */

class ShadowLabyrinthSacrificialAltarGenerator {
  constructor(seed = Date.now()) {
    this.seed = seed;
    this.biome = 'ShadowLabyrinth';
    this.layout = 'SacrificialAltar';
    this.width = 16;
    this.height = 20;
    this.hazardDensity = 0.22;
  }
      ...
      for (let x = 0; x < this.width; x++) {
        if (x === 0 || x === this.width - 1 || y === 0 || y === this.height - 1) {
          ...
          if (rand < this.hazardDensity) {
            grid[y][x] = { tile: 'HAZARD', type: 'shadowlabyrinth_trap', walkable: true };
...
module.exports = { ShadowLabyrinthSacrificialAltarGenerator };
