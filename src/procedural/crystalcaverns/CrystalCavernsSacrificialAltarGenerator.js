/**
 * CrystalCavernsSacrificialAltarGenerator.js - Procedural Dungeon Room Generator: CrystalCaverns -> SacrificialAltar.
 */

class CrystalCavernsSacrificialAltarGenerator {
  constructor(seed = Date.now()) {
    this.seed = seed;
    this.biome = 'CrystalCaverns';
    this.layout = 'SacrificialAltar';
    this.width = 16;
    this.height = 17;
    this.hazardDensity = 0.11;
  }
      ...
      for (let x = 0; x < this.width; x++) {
        if (x === 0 || x === this.width - 1 || y === 0 || y === this.height - 1) {
          ...
          if (rand < this.hazardDensity) {
            grid[y][x] = { tile: 'HAZARD', type: 'crystalcaverns_trap', walkable: true };
...
module.exports = { CrystalCavernsSacrificialAltarGenerator };
