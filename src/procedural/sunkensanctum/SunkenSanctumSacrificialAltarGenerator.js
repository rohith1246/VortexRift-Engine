/**
 * SunkenSanctumSacrificialAltarGenerator.js - Procedural Dungeon Room Generator: SunkenSanctum -> SacrificialAltar.
 */

class SunkenSanctumSacrificialAltarGenerator {
  constructor(seed = Date.now()) {
    this.seed = seed;
    this.biome = 'SunkenSanctum';
    this.layout = 'SacrificialAltar';
    this.width = 16;
    this.height = 23;
    this.hazardDensity = 0.13;
  }
      ...
      for (let x = 0; x < this.width; x++) {
        if (x === 0 || x === this.width - 1 || y === 0 || y === this.height - 1) {
          ...
          if (rand < this.hazardDensity) {
            grid[y][x] = { tile: 'HAZARD', type: 'sunkensanctum_trap', walkable: true };
...
module.exports = { SunkenSanctumSacrificialAltarGenerator };
